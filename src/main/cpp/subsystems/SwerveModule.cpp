...
feet_per_second_t SwerveModule::_GetWheelSpeed() {
    return WHEEL_RADIUS * radians_per_second_t{_drive_motor.GetVelocity().GetValue() / DRIVE_GEAR_RATIO} / 1_rad / DRIVE_RATIO_SCALE;
}

inch_t SwerveModule::_GetWheelPosition() {
    return WHEEL_RADIUS * radian_t{_drive_motor.GetPosition().GetValue() / DRIVE_GEAR_RATIO} / 1_rad / DRIVE_RATIO_SCALE;
}
